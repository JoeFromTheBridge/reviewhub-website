...
from PIL import Image as PILImage  # avoid name clash with ORM Image model
...
from data_export_service import get_data_export_service  # <-- fixed import
...
load_dotenv()
app = Flask(__name__)
logger = app.logger  # use Flask's logger so messages show in Render

# -------------------------
# Robust DB URL handling
# -------------------------
# Prefer SQLALCHEMY_DATABASE_URI, fall back to DATABASE_URL, else SQLite
db_url = (os.getenv("SQLALCHEMY_DATABASE_URI") or os.getenv("DATABASE_URL") or "").strip()

# Normalize legacy postgres prefix
if db_url.startswith("postgres://"):
    db_url = db_url.replace("postgres://", "postgresql+psycopg2://", 1)

# Validate URL; if invalid or empty, fall back to SQLite (ephemeral on Render)
def _valid_sqlalchemy_url(url: str) -> bool:
    try:
        if not url:
            return False
        make_url(url)  # raises on invalid
        return True
    except Exception:
        return False

if not _valid_sqlalchemy_url(db_url):
    logger.warning(
        "No valid DATABASE_URL/SQLALCHEMY_DATABASE_URI provided. "
        "Falling back to SQLite (ephemeral)."
    )
    db_url = "sqlite:///reviewhub.db"
...
# Parse extra origins from env (optional, comma-separated)
...
# Allow any *.vercel.app (covers preview + production on Vercel)
...
def _origin_is_allowed(origin: str) -> bool:
    ...
    if origin in local_origins:
        return True
    if origin in cors_origins:           # anything you listed in env
        return True
    if vercel_regex.match(origin):       # any preview/prod on vercel.app
        return True
    return False

# You use JWT via Authorization header, so credentials aren't required.
# Keep supports_credentials=False; we'll mirror the allowed Origin below.
CORS(
    app,
    resources={r"/*": {"origins": "*"}},
...
def add_cors_headers(resp):
    ...
    try:
        if _origin_is_allowed(origin):
            ...
            # If you ever switch to cookie auth, also add:
...
class DataDeletionRequest(db.Model):
    ...
    deletion_summary = db.Column(db.JSON, nullable=True)
...
class DataExportRequest(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    export_format = db.Column(db.String(10), default="json")
    status = db.Column(db.String(20), default="pending")
    requested_at = db.Column(db.DateTime, default=datetime.utcnow)
...
def verify_email():
    try:
        ...
        return jsonify(
            {"message": "Email verified successfully", "access_token": access_token, "user": user.to_dict()}
        ), 200
...
def resend_verification():
    try:
        ...
        return jsonify({"message": "Verification email sent successfully", "email_sent": email_sent}), 200
...
def forgot_password():
    try:
        ...
        return jsonify({"message": "If an account with this email exists, a password reset link has been sent.",
...
def search_products():
    ...
    results = search_service.search_products(query=query, filters=filters, sort_by=sort_by, page=page, per_page=per_page)
...
def search_reviews():
    ...
    filters = {k: v for k, v in filters.items() if v is not None}
    results = search_service.search_reviews(query=query, filters=filters, sort_by=sort_by, page=page, per_page=per_page)
    return jsonify(results)

@app.route("/api/search/suggestions", methods=["GET"])
def search_suggestions():
    query = request.args.get("q", "")
    suggestion_type = request.args.get("type", "products")
    suggestions = search_service.get_suggestions(query, suggestion_type)
    return jsonify({"suggestions": suggestions})

# -------------------------
# File uploads / Image model from register_image_routes
# -------------------------
app.config["MAX_CONTENT_LENGTH"] = int(os.getenv("MAX_CONTENT_LENGTH", 16777216))  # 16MB
app.config["UPLOAD_FOLDER"] = os.getenv("UPLOAD_FOLDER", "uploads")

Image = register_image_routes(app, db)  # returns ORM Image model

# Attach relationship for Review -> Image (reviews)
Review.image_records = db.relationship(
    "Image",
...
# -------------------------
# Product & Review routes (original + enhanced)
# -------------------------
...
def vote_review(review_id):
    try:
        ...
        if existing_vote:
            ...
            vote = ReviewVote(user_id=user_id, review_id=review_id, is_helpful=is_helpful)
            db.session.add(vote)
...
def create_category():
    try:
        ...
        category = Category(name=data["name"], slug=slug, description=data.get("description"), icon_url=data.get("icon_url"))
        ...
        return jsonify(
            {
                ...
                "category": {"id": category.id, "name": category.name, "slug": category.slug, "description": category.description, "icon_url": category.icon_url},
            }
        ), 201
    except Exception as e:
        db.session.rollback()
...
# -------------------------
# Admin: bulk reindex and dashboards
# -------------------------
...
def reindex_data():
    try:
        ...
        return jsonify({"message": "Reindexing completed successfully", "products_indexed": len(products), "reviews_indexed": len(reviews)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_user_recommendations():
    try:
        ...
        return jsonify({"recommendations": recommendations, "user_id": user_id}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_similar_products(product_id):
    try:
        ...
        return jsonify({"similar_products": similar_products, "product_id": product_id}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_trending_products():
    try:
        ...
        return jsonify({"trending_products": trending_products, "category_id": category_id}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_product_enhanced(product_id):
    try:
        ...
        return jsonify({"product": product.to_dict(), "similar_products": similar_products})
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_bulk_update_products():
    try:
        ...
        return jsonify({"message": f"Updated {updated_count} products successfully", "updated_count": updated_count}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_bulk_update_reviews():
    try:
        ...
        return jsonify({"message": f"Updated {updated_count} reviews successfully", "updated_count": updated_count}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def clear_cache():
    try:
        ...
        return jsonify({"message": f"Cleared {deleted_count} cache entries", "deleted_count": deleted_count}), 200
    except Exception as e:
...
# -------------------------
# Cached product & category endpoints (note: shadows earlier /api/products GET)
# -------------------------
@app.route("/api/products", methods=["GET"])
def get_products_cached():
    try:
        ...
        cache_key = performance_svc.generate_cache_key("products", page, per_page, category_id, sort_by)
...
def record_consent():
    try:
        ...
        success = gdpr_svc.record_consent(user_id, consent_enum, granted, ip_address, user_agent)
        if success:
...
def withdraw_consent():
    try:
        ...
        success = gdpr_svc.withdraw_consent(user_id, consent_enum, ip_address, user_agent)
        if success:
...
def get_export_requests():
    try:
        user_id = get_jwt_identity()
        export_svc = get_data_export_service(db)
        requests_ = export_svc.get_export_requests(user_id)
        return jsonify({"export_requests": requests_}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_cleanup_exports():
    try:
        ...
        return jsonify({"message": f"Cleaned up {cleaned_count} expired export files", "cleaned_count": cleaned_count}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_export_stats():
    try:
        ...
        recent_requests = DataExportRequest.query.order_by(DataExportRequest.requested_at.desc()).limit(10).all()
        recent_data = []
        ...
        stats = {
            ...
            "status_breakdown": {"pending": pending_requests, "completed": completed_requests, "failed": failed_requests},
            "format_breakdown": {"json": json_requests, "csv": csv_requests, "pdf": pdf_requests},
...
def update_privacy_settings():
    try:
        ...
        updatable_fields = [
            ...
            "recommendation_emails", "allow_analytics", "allow_personalization", "third_party_sharing"
        ]
        ...
        db.session.commit()
        return jsonify({"message": "Privacy settings updated successfully", "updated_fields": updated_fields}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def check_content_visibility():
    try:
        ...
        result = {"visible": visibility.get(content_type, True) or is_own_content, "is_own_content": is_own_content,
...
def update_communication_preferences():
    try:
        ...
        communication_fields = ["email_notifications", "marketing_emails", "review_notifications", "recommendation_emails"]
        updated_fields = []
        ...
        privacy_settings.updated_at = datetime.utcnow()
        db.session.commit()
        return jsonify({"message": "Communication preferences updated successfully", "updated_fields": updated_fields}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"error": str(e)}), 500
...
def visual_search_upload():
    try:
        ...
        return jsonify({"message": message, "search_id": result["filename"], "features_extracted": True}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def visual_search_products():
    try:
            ...
            if not features:
                try:
                    ...
                    if os.path.exists(image_path):
                        ...
                        features = {"product_id": product.id, "features": product_features}
                except Exception as e:
        ...
        similar_products = visual_search_service.search_similar_products(query_features, product_features_list)

        results = []
        for similar in similar_products:
            product = Product.query.get(similar["product_id"])
            if product:
                product_image = Image.query.filter_by(product_id=product.id, image_type="product").first()
        ...
        return jsonify({"results": results, "total_results": len(results), "query_processed": True}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_visually_similar_products(product_id):
    try:
        ...
        product_image = Image.query.filter_by(product_id=product_id, image_type="product").first()
        ...
        if not features:
            try:
                if os.path.exists(product_image.file_path):
                    ...
                    features = {"product_id": product_id, "features": product_features}
                else:
        ...
        other_products = db.session.query(Product, Image).join(
            Image, Product.id == Image.product_id
        ).filter(Image.image_type == "product", Product.id != product_id).all()
            ...
            if not other_features:
                try:
                    if os.path.exists(image.file_path):
                        ...
                        other_features = {"product_id": other_product.id, "features": other_product_features}
                except Exception:
                    continue
            if other_features:
                product_features_list.append(other_features)

        similar_products = visual_search_service.search_similar_products(features["features"], product_features_list)

        results = []
        for similar in similar_products[:10]:
            similar_product = Product.query.get(similar["product_id"])
            if similar_product:
                similar_image = Image.query.filter_by(product_id=similar_product.id, image_type="product").first()
        ...
        return jsonify({"product": {"id": product.id, "name": product.name, "brand": product.brand},
                        "similar_products": results, "total_similar": len(results)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_reindex_visual_features():
    try:
        ...
        return jsonify({"message": "Visual search reindexing completed", "processed_products": processed_count,
                        "errors": error_count, "total_products": len(products_with_images)}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def admin_cleanup_visual_search():
    try:
        ...
        return jsonify({"message": f"Cleaned up {cleaned_count} old files", "cleaned_files": cleaned_count, "retention_days": days}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_voice_search_suggestions():
    try:
        ...
        return jsonify({"suggestions": suggestions, "partial_text": partial_text}), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 500
...
def get_voice_search_analytics():
    try:
        ...
        if not user or not user.is_admin:  # fixed: your model uses is_admin
            return jsonify({"error": "Admin access required"}), 403
...
def voice_search():
    try:
        ...
        results = search_service.search_products(
            query=search_query, filters=filters, sort_by=search_params.get("sort", "relevance"), page=1, per_page=20
        )
